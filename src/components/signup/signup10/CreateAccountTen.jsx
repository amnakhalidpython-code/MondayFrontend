...
const CreateAccountTen = () => {
  ...
  const [boardName] = useState(boardData.boardName || 'First Board');
  const [selectedColumns, setSelectedColumns] = useState(
    boardData.selectedColumns || {
      owner: true, status: true, dueDate: true, priority: false,
      lastUpdated: false, timeline: false, notes: false, budget: false, files: false
    }
  );

  useEffect(() => {
    if (userCategory === 'ngo' || userCategory === 'nonprofit') navigate('/dashboard');
  }, [userCategory, navigate]);

  if (userCategory === 'ngo' || userCategory === 'nonprofit') return null;

  const toggleColumn = (column) => {
    const updated = { ...selectedColumns, [column]: !selectedColumns[column] };
    setSelectedColumns(updated);
    setBoardData(prev => ({ ...prev, selectedColumns: updated }));
  };
      ...
      <div className="bg-white w-full h-full overflow-hidden shadow-2xl flex relative">
        ...
        <div className="w-1/2 flex flex-col justify-between bg-white">
              ...
              <div className="flex flex-wrap gap-3">
                {columns.map(({ id, label, Icon }) => (
                  <button
                    ...
                    className={`h-[42px] px-4 rounded-md flex items-center gap-2 transition-all text-[#323338] ${selectedColumns[id]
          ...
          <div className="sticky bottom-0 px-20 py-6 flex justify-between items-center bg-white ">
            <Link to={userCategory === 'ngo' || userCategory === 'nonprofit' ? '/eight' : '/nine'}>
        ...
        <div className="w-1/2 bg-[#eceff8] flex items-center justify-center">
          <div className="bg-white w-full h-[650px] relative overflow-hidden ml-16 rounded-tl-2xl rounded-bl-2xl" style={{ filter: 'drop-shadow(0px 6px 20px rgba(0, 0, 0, 0.2))', paddingTop: '30px', paddingLeft: '30px' }}>
            <div className="mb-2">
              <h1 className="text-[32px] font-medium text-[#323338] leading-[40px] tracking-[-0.5px] truncate max-w-[600px]" style={{ fontFamily: 'Poppins, Roboto, sans-serif' }}>{boardName}</h1>
            </div>
            ...
            <div className="overflow-x-auto w-full">
              <div className="flex gap-0 min-w-max">
                ...
                <div style={{ width: '240px', flexShrink: 0 }}>
                  <div className="mb-0">
                    <div className="flex items-center mb-3" style={{ height: '24px' }}><div className="h-1.5 w-[120px] bg-[#579bfc] rounded"></div></div>
                    <div className="flex items-center bg-white" style={{ borderLeft: '4px solid #579bfc', borderTop: '0.8px solid #d0d4e4', borderBottom: '0.8px solid #d0d4e4', borderRight: '0.8px solid #d0d4e4', borderTopLeftRadius: '4px', height: '40px', paddingLeft: '12px' }}><div className="h-1 w-20 bg-[#c3c6d4] rounded"></div></div>
                    {[0, 1, 2].map(i => (<div key={i} className="flex items-center bg-white" style={{ borderLeft: '4px solid #579bfc', borderBottom: '0.8px solid #d0d4e4', borderRight: '0.8px solid #d0d4e4', height: '40px', paddingLeft: '12px' }}><div className="h-1 w-20 bg-[#c3c6d4] rounded"></div></div>))}
                    <div className="flex items-center bg-white" style={{ borderLeft: '4px solid rgba(87, 155, 252, 0.5)', borderBottom: '0.8px solid #d0d4e4', borderRight: '0.8px solid #d0d4e4', borderBottomLeftRadius: '4px', height: '40px', paddingLeft: '12px' }}><div className="h-1 w-20 bg-[rgba(103,104,121,0.1)] rounded"></div></div>
                    <div style={{ height: '40px', borderRight: '0.8px solid #d0d4e4' }}></div>
                  </div>
                  <div>
                    <div className="flex items-center mb-3" style={{ height: '24px' }}><div className="h-1.5 w-[120px] bg-[#00c875] rounded"></div></div>
                ...
                <div className="flex-1 min-w-0">
                  <div className="mb-0">
                    ...
                    {[0, 1, 2].map((rowIdx) => (
                      <div key={rowIdx} className="flex" style={{ height: '40px' }}>
                        {selectedColumns.owner && <div className="flex items-center justify-center bg-white" style={{ borderBottom: '0.8px solid #d0d4e4', borderRight: '0.8px solid #d0d4e4', width: '136px', flexShrink: 0 }}><img src="https://cdn1.monday.com/dapulse_default_photo.png" alt="owner" className="w-[24px] h-[24px] rounded-full" /></div>}
                        {selectedColumns.status && <div className="flex items-center justify-center" style={{ borderBottom: '0.8px solid #d0d4e4', borderRight: '0.8px solid #d0d4e4', width: '136px', flexShrink: 0, backgroundColor: rowIdx === 0 ? '#fdab3d' : rowIdx === 1 ? '#00c875' : '#df2f4a' }}><span className="text-[15px] text-white leading-[40px]">{rowIdx === 0 ? 'Working on it' : rowIdx === 1 ? 'Done' : 'Stuck'}</span></div>}
                        {selectedColumns.dueDate && <div className="flex items-center justify-center bg-white gap-1" style={{ borderBottom: '0.8px solid #d0d4e4', borderRight: '0.8px solid #d0d4e4', width: '136px', flexShrink: 0 }}>{rowIdx === 0 && <><span className="text-red-500 text-sm">⚠</span><span className="text-[15px] text-[#323338]">Dec 7</span></>}{rowIdx === 1 && <><span className="text-green-500 text-sm">✓</span><span className="text-[15px] text-[#323338]">Dec 8</span></>}{rowIdx === 2 && <><span className="text-gray-500 text-sm">◐</span><span className="text-[15px] text-[#323338]">Dec 9</span></>}</div>}
                        {selectedColumns.priority && <div className="flex items-center justify-center" style={{ borderBottom: '0.8px solid #d0d4e4', borderRight: '0.8px solid #d0d4e4', width: '136px', flexShrink: 0, backgroundColor: rowIdx === 0 ? '#5eb7f1' : rowIdx === 1 ? '#401694' : '#5559df' }}><span className="text-[15px] text-white leading-[40px]">{rowIdx === 0 ? 'Low' : rowIdx === 1 ? 'High' : 'Medium'}</span></div>}
                        {selectedColumns.lastUpdated && <div className="flex items-center justify-center bg-white gap-1" style={{ borderBottom: '0.8px solid #d0d4e4', borderRight: '0.8px solid #d0d4e4', width: '136px', flexShrink: 0 }}><img src="https://cdn1.monday.com/dapulse_default_photo.png" alt="owner" className="w-5 h-5 rounded-full" /><span className="text-[15px] text-[#323338]">{rowIdx === 0 ? '2h ago' : rowIdx === 1 ? '3h ago' : '4h ago'}</span></div>}
                        {selectedColumns.timeline && <div className="flex items-center justify-center bg-white" style={{ borderBottom: '0.8px solid #d0d4e4', borderRight: '0.8px solid #d0d4e4', width: '136px', flexShrink: 0 }}><div className="h-4 w-24 bg-[#579bfc] rounded-md"></div></div>}
                        {selectedColumns.notes && <div className="flex items-center justify-center bg-white" style={{ borderBottom: '0.8px solid #d0d4e4', borderRight: '0.8px solid #d0d4e4', width: '136px', flexShrink: 0 }}><div className="h-1 w-12 bg-[#c3c6d4] rounded"></div></div>}
                        {selectedColumns.budget && <div className="flex items-center justify-center bg-white" style={{ borderBottom: '0.8px solid #d0d4e4', borderRight: '0.8px solid #d0d4e4', width: '136px', flexShrink: 0 }}><span className="text-[15px] text-[#323338]">${rowIdx === 0 ? '100' : rowIdx === 1 ? '1000' : '500'}</span></div>}
                        {selectedColumns.files && <div className="flex items-center justify-center bg-white" style={{ borderBottom: '0.8px solid #d0d4e4', borderRight: '0.8px solid #d0d4e4', width: '136px', flexShrink: 0 }}><span className="text-[15px] text-[#323338]"><img src="https://dapulse-res.cloudinary.com/image/upload/file.png" alt="files" className="w-[20px] h-[24px] " /></span></div>}
                        <div className="bg-white" style={{ width: '48px', borderBottom: '0.8px solid #d0d4e4' }}></div>
                      </div>
                    ))}
                    <div className="flex" style={{ height: '40px' }}>
                      {Object.keys(selectedColumns).map(key => selectedColumns[key] && <div key={key} className="bg-white" style={{ borderBottom: '0.8px solid #d0d4e4', borderRight: '0.8px solid #d0d4e4', width: '136px', flexShrink: 0 }}></div>)}
                      <div className="bg-white" style={{ width: '48px', borderBottom: '0.8px solid #d0d4e4' }}></div>
                    </div>
                    <div className="flex" style={{ height: '40px' }}>
                      {Object.keys(selectedColumns).map(key => selectedColumns[key] && <div key={key} className="bg-white" style={{ borderBottom: '0.8px solid #d0d4e4', borderRight: '0.8px solid #d0d4e4', width: '136px', flexShrink: 0 }}></div>)}
                  ...
                  <div>
                    <div style={{ height: '24px', marginBottom: '12px' }}></div>
                    <div className="flex" style={{ height: '40px' }}>
                      {Object.keys(selectedColumns).map(key => selectedColumns[key] && <div key={key} className="flex items-center justify-center bg-white" style={{ borderTop: '0.8px solid #d0d4e4', borderBottom: '0.8px solid #d0d4e4', borderRight: '0.8px solid #d0d4e4', width: '136px', flexShrink: 0 }}><span className="text-[15px] text-[#323338]">{key.charAt(0).toUpperCase() + key.slice(1).replace(/([A-Z])/g, ' $1').trim()}</span></div>)}
                      <div className="flex items-center justify-center bg-white" style={{ width: '48px', borderTop: '0.8px solid #d0d4e4', borderBottom: '0.8px solid #d0d4e4' }}><span className="text-[#676879] text-lg font-light">+</span></div>
                    </div>
                    <div className="flex" style={{ height: '40px' }}>
                      {Object.keys(selectedColumns).map(key => selectedColumns[key] && <div key={key} className="bg-white" style={{ borderBottom: '0.8px solid #d0d4e4', borderRight: '0.8px solid #d0d4e4', width: '136px', flexShrink: 0 }}></div>)}

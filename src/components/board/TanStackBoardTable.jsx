        ...
        <>
          ...
          <div className="absolute top-full left-0 mt-1 w-[160px] bg-white shadow-xl rounded-lg border border-gray-200 z-50 overflow-hidden py-2 animate-in fade-in zoom-in-95 duration-100">
             {Object.entries(options).map(([key, opt]) => (
                ...
                >
                  <div className="w-6 h-6 rounded flex-shrink-0" style={{ backgroundColor: opt.bg }} />
        ...
        <div className="w-full bg-[#f7f9fb] pl-10 pr-2 py-2 flex flex-col relative">
             ...
             <div className="bg-white border text-[13px] shadow-sm rounded-sm overflow-hidden border-gray-200">
                ...
                <div className="flex border-b border-gray-200 bg-white text-gray-500">
                    <div className="flex-1 w-[300px] px-3 py-2 border-r border-gray-200 font-normal">Subitem</div>
                    <div className="w-[100px] px-3 py-2 border-r border-gray-200 text-center font-normal">Owner</div>
                    <div className="w-[120px] px-3 py-2 border-r border-gray-200 text-center font-normal">Status</div>
                    <div className="w-[120px] px-3 py-2 border-r border-gray-200 text-center font-normal">Date</div>
                ...
                <div className="flex bg-[#e6f4ff] hover:bg-[#dbeeff] transition-colors group">
                    ...
                    <div className="flex-1 w-[300px] p-1 border-r border-transparent">
                        <input 
                            ...
                            className="w-full h-[28px] px-2 bg-white border border-[#cce5ff] rounded text-[13px] outline-none placeholder-gray-500 focus:border-blue-400 transition-all shadow-sm"
                        />
                    </div>
                    {/* Empty Cells */}
    ...
    return (
        <div className="relative h-full w-full flex items-center justify-center px-2 group">
          <div className="flex items-center gap-1">
            ...
            {type !== 'text' && !isEditing && <Info size={13} className="text-gray-400 ml-0.5" />}

            {/* Three Dots Menu - Visible on Header Hover */}
            {!isEditing && (
                <button
                    ref={menuButtonRef}
                    className={`p-1 hover:bg-gray-200 rounded text-gray-500 transition-opacity ml-1 ${activeMenu === id ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}
                    onClick={(e) => {
                        e.stopPropagation();
...
const TanStackBoardTable = ({ 
  ...
  const columns = useMemo(() => {
    ...
    propColumns.forEach(col => {
       ...
       header: () => (
          ...
          cell: (info) => {
                ...
                return (
                   <div className="flex items-center justify-between px-2 h-full group/cell w-full relative overflow-hidden">
                      <div className="flex items-center gap-1 text-[13px] text-[#323338] min-w-0 flex-1">
                         ...
                         <span className="truncate flex-1 font-normal min-w-0">{val || 'Item'}</span>
                      </div>
                      ...
                      <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-0.5 opacity-0 group-hover/row:opacity-100 transition-opacity bg-white/80 backdrop-blur-sm pl-2 flex-shrink-0">
                         <button className="p-1 hover:bg-gray-200 rounded text-gray-500 flex-shrink-0" title="Start Conversation">
                            <MessageSquarePlus size={15} />
                         </button>
                         <button className="p-1 hover:bg-gray-200 rounded text-gray-500 flex-shrink-0" title="Open Item">
                            <Maximize2 size={14} />
                         </button>
                      </div>
                   </div>
             ...
             if (col.type === 'status') {
               return <StatusCell value={val} options={statusConfig} onChange={(newVal) => onUpdateTask(rowOriginal.id, col.id, newVal)} />
             }
             ...
             if (col.type === 'email') {
               ...
               if (isEditing) {
                 ...
                 return (
                   ...
                   >
                     {val ? (
                       <a href={`mailto:${val}`} className="text-[13px] text-[#0073ea] hover:underline truncate block max-w-full" onClick={(e) => e.stopPropagation()}>
                         {val}
                       </a>
                     ) : (
                       <span className="text-[13px] text-gray-400 truncate">Click to add email</span>
                     )}
                   </div>
                 );
             }
                   ...
                   >
                     {val ? (
                       <>
                         ...
                         <span className="text-[13px] text-[#323338] truncate min-w-0">{val}</span>
                       </>
                     ) : (
                       <span className="text-[13px] text-gray-400 truncate">Click to add phone</span>
                     )}
                   </div>
                 );
             }
             ...
             if (col.title.includes('$')) {
                return <div className="px-2 text-[13px] text-[#323338] text-center w-full truncate overflow-hidden">{val ? `$${Number(val).toLocaleString()}` : ''}</div>
             }
             ...
             if (col.type === 'text' || !col.type) {
               ...
               if (isEditing) {
                ...
                return (
                   ...
                   >
                     {val ? (
                       <span className="text-[13px] text-[#323338] truncate block max-w-full">{val}</span>
                     ) : (
                       <span className="text-[13px] text-gray-400 truncate">Click to add</span>
                     )}
                   </div>
                 );
             }

             return <div className="px-2 text-[13px] text-[#323338] truncate text-center w-full overflow-hidden">{val}</div>
          },
          size: col.width || 140,
       }));
    });
    ...
    cols.push(columnHelper.display({
       ...
       header: () => (
         <div className="relative w-full h-full flex items-center justify-center border-t-2 border-t-transparent hover:bg-gray-50 transition-colors">
            ...
            >
              <Plus size={16} className="text-gray-400 mx-auto cursor-pointer hover:text-gray-600" />
            </button>
            {isAddColumnMenuOpen && (
              <AddColumnMenu 
    ...
    return cols;
  }, [propColumns, hoveredHeader, activeMenu, statusConfig, groupColor, onUpdateTask, isAddColumnMenuOpen, onAddColumn, expandedRows]);
        ...
        <div className="flex bg-white pl-[6px]">
          {table.getHeaderGroups().map(headerGroup => (
             <React.Fragment key={headerGroup.id}>
               ...
               {headerGroup.headers.map((header, index) => {
                  ...
                  const zClass = shouldShowMenu ? 'z-[9998]' : (isLast ? 'z-20' : 'z-10');
                  const overflowClass = shouldShowMenu ? 'overflow-visible' : 'overflow-hidden';
         ...
         <div className="flex flex-col pl-[6px]">
                      ...
                      {row.getVisibleCells().map((cell, index) => (
                         <div 
                           ...
                           style={{ 
                             ...
                             borderRight: index === row.getVisibleCells().length - 1 ? 'none' : '1px solid #d0d4e4'
                           }}
            ...
            <div className="flex h-[36px] items-center border-b border-[#d0d4e4] hover:bg-[#f5f7f9] cursor-text bg-white group/add">
               ...
               <div style={{ width: 36, minWidth: 36, maxWidth: 36 }} className="border-r border-[#d0d4e4] h-full flex items-center justify-center flex-shrink-0">
                  <div className="w-4 h-4 border border-gray-200 rounded-[3px] bg-white"></div>
               </div>
               {/* Matches Name column */}
               <div style={{ width: propColumns.find(c => c.id === 'name')?.width || 200, minWidth: propColumns.find(c => c.id === 'name')?.width || 200, maxWidth: propColumns.find(c => c.id === 'name')?.width || 200 }} className="border-r border-[#d0d4e4] h-full flex items-center px-3 flex-shrink-0 overflow-hidden">
                   <input 
                      type="text" 
                      placeholder="+ Add donor" 
                      className="w-full bg-transparent border-none outline-none text-[13px] text-gray-500 placeholder-gray-500 h-full font-normal group-hover/add:placeholder-gray-700"
                      onKeyDown={(e) => {
                         if(e.key === 'Enter') {
               ...
               <div className="flex h-full">
                  {propColumns.filter(c => c.id !== 'name').map((col, idx) => (
                     <div 
                       key={`spacer-${idx}`} 
                       style={{ width: col.width || 140, minWidth: col.width || 140, maxWidth: col.width || 140 }} 
                       className="h-full border-r border-[#d0d4e4] flex-shrink-0 overflow-hidden"
                     />
                  ))}
              </div>
               <div style={{ width: 36, minWidth: 36, maxWidth: 36 }} className="h-full flex-shrink-0" />
            </div>
            
            {/* Summary Row */}
            <div className="flex h-[40px] items-center bg-white border-b border-[#d0d4e4]">
               <div style={{ width: 36, minWidth: 36, maxWidth: 36 }} className="border-r border-[#d0d4e4] h-full flex-shrink-0" />
                  ...
                  >
                     {(col.id.includes('donated') || col.id === 'grantAmount') && (
                        <div className="flex flex-col items-center justify-center h-full w-full overflow-hidden">
                           <span className="font-semibold text-[#323338] leading-tight truncate">$20,800</span>
                           <span className="text-[10px] text-gray-500 leading-tight truncate">sum</span>
                        </div>
                     )}
                  </div>
               ))}
